...
from gemini_voice.paths import MODELS_DIR, get_model_path
...
def test_get_model_path_redundant_prefix(tmp_path):
    ...
    with patch("gemini_voice.paths.MODELS_DIR", models_dir):
        with patch("gemini_voice.paths.load_config", return_value={"model": "pt_BR-faber-medium.onnx"}):
            path = get_model_path()
            assert path == str(model_file)
            
    # 2. Test with redundant "models/" prefix
    with patch("gemini_voice.paths.MODELS_DIR", models_dir):
        with patch("gemini_voice.config.load_config", return_value={"model": "models/pt_BR-faber-medium.onnx"}):
            path = get_model_path()
            assert path == str(model_file)
            
    # 3. Test with "./models/" prefix
    with patch("gemini_voice.paths.MODELS_DIR", models_dir):
        with patch("gemini_voice.paths.load_config", return_value={"model": "./models/pt_BR-faber-medium.onnx"}):
            path = get_model_path()
            assert path == str(model_file)
